...
namespace AMVCC.Controllers
{
    public class SpawnController : SeaWarElement
    {
        ...
        [SerializeField] private GameObject platformMeshCash;
...
//            Debug.Log(newUnit.unitName + prefab.name);
        ...
        public void FindProperSpawnPositionTowers(Collider hit)
        {
            ...
            Vector3 spawnPosition = hit.transform.position + new Vector3(0,hit.GetComponent<BoxCollider>().bounds.extents.y + 0.56f,0);
...
//            Debug.Log(prefab.name);
                ...
                if (FindObjectOfType<SeaWarUIView>().isRoadsEnabled)
                {
                        ...
                        if (unit.layer == LayerMask.NameToLayer("Sea Crafts")) 
                        {
                            ...
                            else if (unit.CompareTag("Red"))
                            {
                                ...
                                //todo if count of each queue had been more than a huge number, null objects must dequeue with foreach loop and a dequeue
                            }
            ...
            else if (prefab.layer == LayerMask.NameToLayer("Buildings"))
            { 
                    var unit = Instantiate(prefab, spawnPosition,spawnRotation);
                    unit.name = prefab.name;
            ...
            if (prefab.name == "Sea Mine")
            {
                Debug.Log("spawn sea mine");
                var unit = Instantiate(prefab, spawnPosition,spawnRotation);
                unit.name = prefab.name;
                unit.transform.position -= new Vector3(0, unit.GetComponent<CapsuleCollider>().bounds.extents.y, 0);
            }
